...
def update_state(event):
    global  start_time
    if event.event_type == 'down' and not start_time:
        start_time = time.time()    
    if event.event_type == 'down' and event.scan_code == 57: # when space bar released
...
k = keyboard.hook(update_state)
keyboard.wait('esc')
keyboard.unhook(update_state)
